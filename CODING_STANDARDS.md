...
### Content sync (`cmd/content-sync`)

For seeding a fresh homelab deployment whose Postgres isn't reachable from outside its own cluster network — a schema-migrated but content-empty database only reachable through the blog instance itself. Rather than talking to that database directly, `cmd/content-sync` reads every project/post from a source DSN and replays them through the destination instance's own admin HTTP routes (`POST /projects`, `POST /posts`), the same forms a human would fill in. Idempotent: a project/post whose slug already exists on the destination is skipped (detected via the same "already exists" duplicate-slug form error a human would see), not treated as a failure — safe to re-run after a database redeploy without double-creating anything already there. Invoked via `make content/sync dest=<destination-base-url>` (gated behind the same `confirm` prompt as the migration targets, since it writes real content) or `make content/sync/dry-run dest=<...>` to preview first. Post→project links are preserved by scraping the destination's own `/posts/new` checkbox list for its (independently-assigned) project IDs — there's no JSON API for this, only the HTML form itself.
